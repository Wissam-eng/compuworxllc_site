...
class UserController extends Controller
{
        ...
        foreach ($addressList as $key => $value) {
            $list[] = [
              ...
              'name' => $value['name'],
                ...
                'city' => $value->cityData(),
                'governorate' => $value->governorateData(),
                // 'country' => [
    ...
    public function createFavorite(Request $request)
    {
        ...
        if ($product == '') {
            return [
                'status' => 'faild',
                'message' => trans('api.productDoesn\'tExist'),
                'data' => []
            ];
        }

        $checkOldFav = Wishlist::where('product_id',$request->product_id)->where('user_id',$user_id)->first();
        if ($checkOldFav != '') {
            $resArr = [
                'status' => 'success',
                'message' => trans('api.yourDataHasBeenSavedSuccessfully'),
    ...
    public function myFavoriteList(Request $request)
    {
        ...
        $fav = Wishlist::where('user_id',$user_id)->orderBy('id','desc')->get();
        $list = [];
        foreach ($fav as $key => $value) {
            if ($value->apiData($lang)['product'] == '') {
                $favSingle = Wishlist::find($value->id)->delete();
            } else {
