            ...
            <div class="col-lg-6 col-sm-12 col-xs-12">
              ...
              <div class="row check-out">
                ...
                <div class="form-group col-md-6 col-sm-6 col-xs-12">
                  ...
                  <span class="validity text-danger">*</span>
                  {!! Form::text('phone',$order->address != '' ? $order->address->phone : '',['id'=>'phone','placeholder'=>'53000000','required']) !!}
                  @error('phone')
                    <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
            ...
            <div class="col-lg-6 col-sm-12 col-xs-12">
              <div class="checkout-details">
                          ...
                          <h4> {{ $product['name_'.$lang] }} :
                            <span>
                                ...
                                @if ($product->taxTotal() > 0)
                                  ...
                                  <small class="mt-1 d-block text-black-50">{{trans('common.priceWithTax')}}</small>
                                @endif
                            </span>
                          </h4>
                        </li>
                ...
                <div class="payment-box">
                  ...
                  <div class="form-group col-md-12 col-sm-12 col-xs-12">
                    <label class="field-label" for="payment_method">{{ trans('common.payment_method') }}</label>
                    <span class="validity text-danger">*</span>
...
@section('scripts')
  <script>
    $(document).on('keydown','.coupon_text',function(){
      setTimeout(() => {
        var coupon_text = $(this).val();
        $('.coupun_val').val(coupon_text);
      }, 1000);
    });
    $('select#chooseDeliveryMethod').on('change', function() {
      if (this.value == '0') {
        $('#shippingPrice').val("0");
        $('#totalShipping').html("0 "+"{{getDefaultCurrencySypml()}}");
      } else {
        $('#shippingPrice').val("{{getSettingValue('deliveryPrice')}}");
        $('#totalShipping').html("{{getSettingValue('deliveryPrice')}} "+"{{getDefaultCurrencySypml()}}");
      }
    });
  </script>
  <style>
